...
function App() {
  ...
  const discountSetupEnabled = settings?.shop.myshopifyDomain === 'nuphy-store.myshopify.com';
  ...
  useEffect(() => {
    if (!discountSetupEnabled) {
      setDiscountStatus(null); setDiscountError(''); setDiscountBusy(false);
      return;
    }
    let cancelled = false;
    ...
    return () => { cancelled = true; };
  }, [discountSetupEnabled, loadAttempt]);
  ...
  return (
    <s-page heading="BOGO 买赠活动">
      ...
      <s-stack gap="base">
        ...
        {settings && <>
          {discountSetupEnabled && <s-section heading="赠品自动折扣">
            <s-stack gap="base">
