...
export function readSchedule(fields: ScheduleFields, timeZone: string, now = Date.now(), original?: StoredCampaign) {
  ...
  const startsAt = sameStart ? original!.startsAt : fields.start === 'scheduled' ? toInstant(fields.startDate, fields.startTime, timeZone) : undefined;
  const endsAt = sameEnd ? original!.endsAt : fields.hasEnd ? toInstant(fields.endDate, fields.endTime, timeZone) : undefined;
  // 历史“立即开始”活动没有保存开始时刻，改名或停用时不能拿现在当原开始时间。
  if (endsAt && (startsAt ? Date.parse(endsAt) <= Date.parse(startsAt)
    : !(sameStart && sameEnd) && Date.parse(endsAt) <= now)) {
